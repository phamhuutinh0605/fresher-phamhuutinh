...
const DetailProduct = () => {
  ...
  const handleChangeQuantity = (event:React.ChangeEvent<HTMLInputElement>) => {
    ...
    dispatch(changeQuantity({
      id: String(id), quantity: Number(event.target.value)
    }))
  }
  ...
  const handleAddToCart = () => {
    dispatch(addToCart({
      id: String(id), title: String(title), price: Number(price), desc: String(desc), image, quantity: Number(quantity)
    }))
  }
  ...
  const handleFilterPost = () => {
    return posts.filter((item: Post) => item.idProduct[0] === id || item.idProduct[1] === id || item.idProduct[2] === id)
  }
  return (
    <>
      <Header />
        ...
        <div className="product__bg">
          <div className="detail__content shopee__container">
            <div className="detail__left">
              <Image alt="green iguana" src={String(image)} width={300} height={100} />
            </div>
            <div className="detail__right">
              <ProductInformation title={title} price={price} quantity={quantity} handleChangeQuantity={handleChangeQuantity} handleAddToCart={handleAddToCart}/>
            </div>
          </div>
